	...
	-- < convert(date, dateadd(d,-1,dateinsert_scoreid)) - на вчера
	-- < convert(date, dateadd(d,-3,t.dateinsert_scoreid)) - с лагом для транзакций
	...
	into  #temp2
	from risk_test.dbo.temp1 t
		join FPD_MOB2 as f on isnull(t.dealno, '') = isnull(f.legalcontractnum, '') and t.dealid = f.contractid
	...
	into #temp2_provodki_join
	  from risk_test.dbo.temp2 t
    left join Provodki p on t.dealid=p.contractid and p.operdate < t.date_of_calculation
...
-----------------------------------------------------------------------------------------------------
 ---------------------------- Характеристики користування картою ---------------------------- 
	...
	into Risk_test.[dbo].[Scoring_DataMart_CC_trans]
	from #temp2 as t 
